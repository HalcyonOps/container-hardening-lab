...
## VEX — Trivy's native `--vex` over a separate VEX consumer

**Chosen:** [OpenVEX](https://openvex.dev/) documents, generated by `scripts/generate_vex.py` and consumed via `trivy image --vex`.

Trivy already reads OpenVEX natively; adding a separate VEX tool to interpret the document would be a second scanner-adjacent dependency for no benefit. The document itself is **generated, not hand-maintained** — `scripts/generate_vex.py` re-derives it from the Trivy report and [known-findings.md](known-findings.md) on every run, and `scripts/verify-vex.sh` proves in CI that applying it suppresses only the CVE it claims to and nothing else. A hand-written VEX file would be exactly the suppression-that-nobody-reads failure this repo's register exists to avoid, just moved into a different file format.

Only findings that are true of the *artifact*, for every consumer, are eligible — see the `VEX_ELIGIBLE` allowlist in `scripts/generate_vex.py` and the reasoning under CVE-2025-69720 in [known-findings.md](known-findings.md).
